...
public class AI_Mandrake : AI //使用以自身位置為中心的範圍攻擊
{
    public override void StartAI(Action callback)
    {
        if (!_myself.Info.HasUseSkill) //還沒用過技能
        {
            ...
            if (target != null)
            {
                if (Utility.GetDistance(transform.position, target.transform.position) <= _selectedSkill.Data.Range_1 - 1) //目標已在射程內,無需移動
                {
                    CanHitTarget = true;
                    _myself.SetTarget(Vector2Int.RoundToInt(transform.position)); //技能的施放位置是自己
                ...
                else if (_myself.Info.MOV > 0)//需要移動
                {
                    ...
                    List<Vector2Int> positionList = Utility.GetRhombusPositionList(_myself.SelectedSkill.Data.Range_1 - 1, Vector2Int.RoundToInt(target.transform.position), false); //技能可打中目標的位置
                    ...
                    {
                        CanHitTarget = false;
                        List<Vector2Int> tempList = BattleFieldManager.Instance.GetPath(transform.position, Vector2Int.RoundToInt(target.transform.position), BattleCharacterInfo.CampEnum.None);
