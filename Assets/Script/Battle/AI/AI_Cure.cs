...
public class AI_Cure : AI
{
    public override void StartAI(Action callback) //與 AI_Single 相同
    {
        if (!_myself.Info.HasUseSkill) //還沒用過技能
        {
            ...
            if (target != null)
            {
                ...
                {
                    CanHitTarget = true;
                    _myself.GetSkillRange(out Vector2Int targetPosition, out List<Vector2Int> rangeList);
                    callback();
                }
                else if (_myself.Info.MOV > 0)//需要移動
                {
                    ...
                    {
                        CanHitTarget = false;
                        List<Vector2Int> tempList = new List<Vector2Int>(_myself.GetPath(_myself.TargetPosition));
                        ...
                        for (int i = 1; i < tempList.Count; i++) //擷取小於等於移動距離的路徑
                        {
                            ...
                            if (pathLength <= _myself.Info.MOV && BattleController.Instance.GetCharacterByPosition(tempList[i]) == null)
                            {
    ...
    protected BattleCharacter GetTarget()
    {
        ...
        List<Vector2Int> detectRangeList = new List<Vector2Int>();
        ...
        detectRangeList = _myself.GetDetectRange();
        ...
        if (inRangeList.Count > 0) //有可以治療的對象
        {
            ...
            if (candidateList.Count > 0)
            {
                ...
                {
                    if (candidateList[i].Info.CurrentHP < target.Info.CurrentHP) //挑血少的
                    {
